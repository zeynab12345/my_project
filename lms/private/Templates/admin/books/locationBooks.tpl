...
{if $location != null}
    {assign var=locationName value=$location->getName()}
{else}
    {assign var=locationName value=''}
{/if}
{block name=title}{t name=$locationName}Location %1 Books{/t}{/block}
{block name=toolbar}
{/block}
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card" id="books-block">
                {include 'admin/books/locationBooks-list.tpl'}
            </div>
        </div>
    </div>
{/block}
...
{block name=footerCustomJs append}
    <script>
        ...
        $(document).on('change', '#countPerPage', function (e) {
            var url = '{$routes->getRouteString("locationBooksView")}';
            $.ajax({
                type: "POST",
                url: url.replace('[locationId]', {if $location != null}{$location->getId()}{/if}),
