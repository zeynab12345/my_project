                ...
                <th class="d-none header-bulk-actions">
                    <div class="btn-group" role="group">
                        <button class="btn btn-danger btn-sm" type="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            <i class="far fa-trash-alt mr-1"></i> {t}Delete Books{/t}
                        </button>
                        <ul class="dropdown-menu delete-dropdown text-regular">
                            {if $isDemoMode === true}
                                ...
                                <li class="divider"></li>
                            {/if}
                            <li>{t}Do you really want to delete?{/t}</li>
                            <li class="divider"></li>
                            <li>
                                <button class="btn btn-outline-danger delete-books" {if $isDemoMode === true}disabled{/if}>
                                    <span class="btn-icon"><i class="far fa-trash-alt"></i></span> {t}Delete{/t}
                                </button>
                            </li>
                        </ul>
                        <button class="btn btn-primary btn-sm" id="gen-barcode"><i class="fas fa-magic mr-1"></i> {t}Generate Barcode{/t}</button>
                        <button class="btn btn-success btn-sm" data-toggle="modal" data-target="#bulkEditBooks"><i class="fas fa-pencil-alt mr-1"></i> {t}Edit Books{/t}</button>
                    </div>
                </th>
            ...
            {if isset($books) and $books != null}
                {foreach from=$books item=book name=book}
                    <tr>
                        ...
                        <td>
                            ...
                            {if $book->getEBookId() != null}
                                <div class="book-list-info">
                                    ...
                                    <a href="{$routes->getRouteString("electronicBookGet",["electronicBookId"=>$book->getEBookId()])}" class="ml-1"><i class="far fa-hdd"></i> {t}Download{/t}
                                    </a>
                                    <a href="{$routes->getRouteString("electronicBookViewAdmin",["bookId"=>$book->getId()])}" class="ml-1"><i class="fas fa-glasses"></i> {t}Read{/t}
                                    </a>
                                </div>
                            {/if}
                        </td>
                        ...
                        <td class="text-center">
                            <a href="{$routes->getRouteString("bookEdit",["bookId"=>$book->getId()])}" class="btn btn-outline-info btn-sm no-border {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}Edit{/t}"><i class="fas fa-pencil-alt"></i></a>
                            <a href="{$routes->getRouteString("bookClone",["bookId"=>$book->getId()])}" class="btn btn-outline-info btn-sm no-border {if $activeLanguage->isRTL()}ml-1{else}mr-1{/if}" data-container="body" data-toggle="tooltip" title="{t}Clone{/t}"><i class="far fa-clone"></i></a>
