<?php
    ...
    class Yahoo extends AbstractProvider {
        ...
        public function getIdentity(AccessTokenInterface $accessToken) {
            $response = $this->httpClient->request($this->getBaseUri() . "user/{$accessToken->getUserId()}/profile",
                                                   [ 'format' => 'json', ],
                                                   Client::GET,
            ...
            if (isset( $result->birthdate )) {
                $result->birth_date = date('Y-m-d',
                                           strtotime($result->birthdate . '/' . $result->birthYear));
            }

            $hydrator = new ObjectMap([ 'guid'       => 'id',
                                        'image'      => 'picture',
                                        'email'      => 'email',
                                        'givenName'  => 'firstname',
                                        'familyName' => 'lastname',
