<?php
    ...
    class PHPMailer {
        ...
        public function IsSendmail() {
            if (!stristr(ini_get('sendmail_path'),
                         'sendmail')
            ) {
        ...
        public function AddReplyTo($address, $name = '') {
            return $this->AddAnAddress('Reply-To',
                                       $address,
                                       $name);
        }
        ...
        protected function AddAnAddress($kind, $address, $name = '') {
            if (!preg_match('/^(to|cc|bcc|Reply-To)$/',
                            $kind)
            ) {
        ...
        public static function ValidateAddress($address) {
            if (( defined('PCRE_VERSION') ) && ( version_compare(PCRE_VERSION,
                                                                 '8.0') >= 0 )
            ) {
                ...
                switch ($this->Mailer) {
                    case 'sendmail':
                        return $this->SendmailSend($this->MIMEHeader,
                                                   $this->MIMEBody);
                    case 'smtp':
                        return $this->SmtpSend($this->MIMEHeader,
                                               $this->MIMEBody);
                    case 'mail':
        ...
        public function SmtpConnect() {
            ...
            try {
                while ($index < count($hosts) && !$connection) {
                    ...
                    if ($this->smtp->Connect(( $ssl ? 'ssl://' : '' ) . $host,
                                             $port,
                                             $this->Timeout)
                    ) {
        ...
        public function CreateHeader() {
            ...
            if (count($this->cc) > 0) {
                $result .= $this->AddrAppend('Cc',
                                             $this->cc);
            }

            // sendmail and mail() extract Bcc from the header before sending
            if (( ( $this->Mailer == 'sendmail' ) || ( $this->Mailer == 'mail' ) ) && ( count($this->bcc) > 0 )) {
                $result .= $this->AddrAppend('Bcc',
                                             $this->bcc);
            }

            if (count($this->ReplyTo) > 0) {
                $result .= $this->AddrAppend('Reply-To',
                                             $this->ReplyTo);
            }
            ...
            if ($this->MessageID != '') {
                ...
                $result .= sprintf("Message-ID: <%s@%s>%s",
                                   $uniq_id,
                                   $this->ServerHostname(),
                                   $this->LE);
            }
            $result .= $this->HeaderLine('X-Priority',
                                         $this->Priority);
            if ($this->XMailer == '') {
            ...
            switch ($this->message_type) {
                ...
                case 'attach':
                    ...
                    $body .= $this->EncodeString($this->Body,
                                                 $this->Encoding);
                    $body .= $this->LE . $this->LE;
                    $body .= $this->AttachAll("attachment",
                                              $this->boundary[1]);
                    break;
                case 'inline_attach':
            ...
            elseif ($this->sign_key_file) {
                try {
                    ...
                    file_put_contents($file,
                                      $body); //TODO check this worked
                    $signed = tempnam("",
                                      "signed");
                    if (@openssl_pkcs7_sign($file,
                                            $signed,
        ...
        public function EncodeQP($string, $line_max = 76, $space_conv = false) {
            ...
            if (!in_array('convert.*',
                ...
                return $this->EncodeQPphp($string,
                                          $line_max,
                                          $space_conv); //Fall back to old implementation
            }
            $fp = fopen('php://temp/',
                        'r+');
            $string = preg_replace('/\r\n?/',
            ...
            if (isset( $images[2] )) {
                foreach ($images[2] as $i => $url) {
                    ...
                    if (!preg_match('#^[A-z]+://#',
                        ...
                        if (strlen($directory) > 1 && substr($directory,
                                                             -1) != '/'
                        ) {
                            $directory .= '/';
                        }
        ...
        public static function _mime_types($ext = '') {
            $mimes = array( 'xl'    => 'application/excel',
        ...
        public function DKIM_Add($headers_line, $subject, $body) {
            ...
            $from = str_replace('|',
                                '=7C',
                                $this->DKIM_QP($from_header));
            $to = str_replace('|',
                              '=7C',
                              $this->DKIM_QP($to_header));
            $subject = str_replace('|',
                                   '=7C',
            ...
            $DKIMb64 = base64_encode(pack("H*",
                                          sha1($body))); // Base64 of packed binary SHA-1 hash of body
