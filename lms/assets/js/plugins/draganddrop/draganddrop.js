...
Sortable.prototype.serialize = function(container) {
    ...
    return container.children(self.options.nodes).not(self.options.container).map(function(ix, el) {
        ...
        var $el = $(el),
            ...
            id = $el.attr('id'),
            formEl = $el.clone().children('ul').remove().end().find('input,select,textarea'),
            node = {id: id || text };
            var formData = getFormData(jQuery(formEl)); // store json object
            node = $.extend({}, node, formData); // Combine the two objects
